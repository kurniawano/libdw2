...
class SensorInput:
    ...
    """
    def __init__(self, app, cheat = False):
        """
        ...
        the real robot.
        """
        self.sonars = app.storedsonars.get()[:]
        """List of 8 sonar readings, in meters."""
        if cheat == True:
            self.odometry = \
                util.valueListToPose(app.abspose.get())
            ...
            self.odometry = \
                util.valueListToPose(app.odpose.get())
        self.analogInputs = app.analogInputs.get()
...
class Action:
    ...
    """
    def __init__(self, app, fvel = 0.0, rvel = 0.0, 
    ...
    def execute(self):
        if self.discreteStepLength:
            app.discreteMotorOutput(self.fvel, self.rvel,
                                                self.discreteStepLength)
        else:
            app.motorOutput(self.fvel, self.rvel)
        app.analogOutput(self.voltage)
